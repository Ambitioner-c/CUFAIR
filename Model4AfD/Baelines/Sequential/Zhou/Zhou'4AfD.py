...
class ZhouModel(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        # Consensus
        consensus = self.bigru(
            torch.cat([bert_output_right.unsqueeze(1).expand(-1, comment.size(1), -1), bert_output_comment], dim=-1)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Zhou Model for AfD')
    parser.add_argument('--task_name', nargs='?', default='Zhou_BiGRU4AfD',
                        help='Task name')
