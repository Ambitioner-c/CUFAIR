...
class OurModel(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        # SC
        source_credibility = self.gru(bert_output_comment)[:, -1, :]                               # torch.Size([batch_size, 64])
...
def parse_args():
    parser = argparse.ArgumentParser(description='GRU w/o QA Model')
    parser.add_argument('--task_name', nargs='?', default='GRU_wo_QA',
                        help='Task name')
