...
class RaoModel(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        o_r = torch.stack(o_r, dim=1)                                                               # torch.Size([batch_size, seq_length, hidden_size])

        consensus = self.lstm_sentence(o_r)[:, -1, :]                                               # torch.Size([batch_size, hidden_size])

        outputs = self.consensus_layer(consensus)                                                   # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Rao Model for AfD')
    parser.add_argument('--task_name', nargs='?', default='Rao4AfD',
                        help='Task name')
