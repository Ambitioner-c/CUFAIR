...
class AVERModel(nn.Module):
    def __init__(
        ...
        self.consensus_layer = nn.Linear(hidden_size * 4, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        v_a = bert_output_right                                                                     # torch.Size([batch_size, hidden_size])
        v_s = self.q_2_e_attention(bert_state_right, bert_state_comment)                            # torch.Size([batch_size, seq_length, hidden_size])

        # Evidence Sentence Ranking
        # QA-guided Evidence Ranking
        beta = torch.sigmoid(
            torch.einsum('bsd,bd->bs', v_s, self.w3(v_a)))                                     # torch.Size([batch_size, seq_length])
        v_s_bar = torch.einsum('bs,bsd->bsd', beta, v_s)                                       # torch.Size([batch_size, seq_length, hidden_size])

        # Evidence Agreement Matching
        gamma = torch.softmax(
            torch.max(torch.einsum('ad,bsd->bas', self.w4, torch.tanh(self.w5(v_s_bar))), dim=1
                      )[0], dim=-1)                                                                  # torch.Size([batch_size, 3, seq_length])
        ...
        outputs = self.consensus_layer(
            torch.cat([v_a, v_s_tilde, torch.abs(v_a - v_s_tilde), v_a * v_s_tilde], dim=-1)) # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='AVER Model for AfD')
