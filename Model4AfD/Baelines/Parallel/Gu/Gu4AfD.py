...
class GuModel(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        ad_hoc = self.snippet_layer(
            torch.cat(
                (bert_output_right.unsqueeze(1).expand(-1, 10, -1), bert_output_comment), dim=-1)
        )                                                                                           # torch.Size([batch_size, seq_length, hidden_size])
        ad_hoc = self.self_attention(ad_hoc)[0]                                                     # torch.Size([batch_size, seq_length, hidden_size])

        outputs = self.consensus_layer(ad_hoc.view(ad_hoc.size(0), -1))                             # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Gu Model for AfD')
    parser.add_argument('--task_name', nargs='?', default='Gu4AfD',
                        help='Task name')
