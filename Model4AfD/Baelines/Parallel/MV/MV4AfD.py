...
class MVModel(nn.Module):
    def __init__(
        ...
        self.consensus_layer = nn.Linear(hidden_size * 10, hidden_size)

        self.fusion_layer = nn.Linear(hidden_size * 2, hidden_size)

        self.verification_score_layer = nn.Linear(hidden_size * 2, num_labels)

        self.confidence_score_layer = nn.Linear(10 * 10 * 128 * 128, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        bert_state_comment = torch.stack(bert_state_comment,dim=0)                                  # torch.Size([batch_size, seq_length, max_length, hidden_size])

        # Verification Component
        # Pivot Supporting Evidence
        E = torch.einsum('bmih,bnjh->bmnij', bert_state_comment, bert_state_comment)          # torch.Size([batch_size, seq_length, seq_length, max_length, max_length])
        ...
        s_v = self.verification_score_layer(torch.cat([a, g], dim=-1))                       # torch.Size([batch_size, num_labels])

        # Confidence-based Combination of Matching and Verification
        c = self.confidence_score_layer(E.view(E.size(0), -1))                                      # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Matching-Verification for AfD')
    parser.add_argument('--task_name', nargs='?', default='MV4AfD',
                        help='Task name')
