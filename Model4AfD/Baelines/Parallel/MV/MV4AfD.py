...
class MVModel(nn.Module):
    def __init__(
        ...
        self.matching_layer = nn.Linear(hidden_size * 2, num_labels)

        self.consensus_layer = nn.Linear(hidden_size * 5, hidden_size)
    ...
    def forward(self, inputs: dict):
        ...
        bert_left = self.bert(text_left)
        bert_right = self.bert(text_right)
        bert_output_left = self.reduction_layer(bert_left['pooler_output'])              # torch.Size([batch_size, hidden_size])
        bert_output_right = self.reduction_layer(bert_right['pooler_output'])            # torch.Size([batch_size, hidden_size])
        bert_state_right = self.reduction_layer(bert_right['last_hidden_state'])         # torch.Size([batch_size, max_length, hidden_size])

        bert_state_comment = []
        for j in range(len(comment)):
            bert_comment = self.bert(comment[j])
            bert_state_comment.append(self.reduction_layer(bert_comment['last_hidden_state']))
        bert_state_comment = torch.stack(bert_state_comment,dim=0)                                  # torch.Size([batch_size, max_sequence_length, max_length, hidden_size])

        # Matching Component
        s_r = self.matching_layer(
            torch.cat([bert_output_left, bert_output_right], dim=-1))                        # torch.Size([batch_size, 2])

        # Verification Component
        # Pivot Supporting Evidence
        E = torch.einsum('bmih,bnjh->bmnij', bert_state_comment, bert_state_comment)          # torch.Size([batch_size, max_sequence_length, max_sequence_length, max_length, max_length])
        E = nn.Softmax(dim=-1)(E)
        p = E.mean(dim=[-1, -2, -3])
        F_p = bert_state_comment.gather(dim=1, index=p.argmax(dim=1).unsqueeze(1).unsqueeze(2).unsqueeze(3).expand(-1, 1, bert_state_comment.size(2), bert_state_comment.size(3))).squeeze(1)

        # Consensus Representation
        G = self.pivoted_attention(F_p, bert_state_comment)
        G = G.permute(0, 2, 1, 3).reshape(-1, bert_state_comment.size(2), bert_state_comment.size(1) * bert_state_comment.size(3))
        G = self.consensus_layer(G)                                                                 # torch.Size([batch_size, max_length, hidden_size])

        # Answer-consensus Verification
        E_AG = torch.einsum('bih,bjh->bij', bert_state_right, G)
        A_ = torch.matmul(nn.Softmax(dim=-1)(E_AG), G)                                              # torch.Size([batch_size, max_length, hidden_size])
        G_ = torch.matmul(nn.Softmax(dim=-1)(E_AG.permute(0, 2, 1)), bert_state_right)              # torch.Size([batch_size, max_length, hidden_size])
        M_A = self.fusion_layer(torch.cat([bert_state_right, A_], dim=-1))                   # torch.Size([batch_size, max_length, hidden_size])
        M_G = self.fusion_layer(torch.cat([G, G_], dim=-1))                                  # torch.Size([batch_size, max_length, hidden_size])
        a, _ = torch.max(M_A, dim=1)                                                                # torch.Size([batch_size, hidden_size])
        g, _ = torch.max(M_G, dim=1)                                                                # torch.Size([batch_size, hidden_size])
        s_v = self.verification_score_layer(torch.cat([a, g], dim=-1))                       # torch.Size([batch_size, 2])

        # Confidence-based Combination of Matching and Verification
        c = self.confidence_score_layer(E.view(E.size(0), -1))                                      # torch.Size([batch_size, 2])

        outputs = (s_r + c * s_v)[:, 1].unsqueeze(1)

        return outputs

    @staticmethod
    def pivoted_attention(x, y):
        """

        :param x: pivot [B, S, D]
        :param y: evidences [B, N, S, D]
        :return:
        """
        attention_scores = torch.einsum('bsd,bnsd->bn', x, y)                    # [B, N]

        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        context_layer = torch.einsum('bn,bnsd->bnsd', attention_probs, y)        # [B, N, S, D]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Matching-Verification')
    parser.add_argument('--task_name', nargs='?', default='MV',
                        help='Task name')
