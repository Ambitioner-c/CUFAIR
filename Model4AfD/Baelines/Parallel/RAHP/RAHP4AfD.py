...
class RAHPModel(nn.Module):
    def __init__(
        ...
        self.consensus_layer = nn.Linear(hidden_size * 10, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        o_r = []
        for j in range(len(comment)):
            o_r.append(self.bilstm_ra(bert_state_comment[j])[:, -1, :])                            # torch.Size([seq_length, hidden_size])
        o_r = torch.stack(o_r, dim=0)                                                              # torch.Size([batch_size, seq_length, hidden_size])

        o_q = self.reduction_layer(self.bert(text_right)['pooler_output'])                         # torch.Size([batch_size, hidden_size])
        v_r = self.q_2_r_attention(o_q, bert_state_comment)                                        # torch.Size([batch_size, seq_length, hidden_size])

        m_r = torch.add(v_r, o_r)                                                                  # torch.Size([batch_size, seq_length, hidden_size])

        s_ra = self.mlp_ra_layer(
            torch.cat([m_r, m_a.unsqueeze(1).repeat(1, m_r.size(1), 1)], dim=-1))           # torch.Size([batch_size, seq_length, hidden_size])

        outputs = self.consensus_layer(s_ra.view(s_ra.size(0), -1))                                # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='RAHP Model for AfD')
    parser.add_argument('--task_name', nargs='?', default='RAHP4AfD',
                        help='Task name')
