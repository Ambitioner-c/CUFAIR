...
class OurModel(nn.Module):
    def __init__(
        ...
        self.credibility_layer = nn.Linear(hidden_size * 7, 64)
    ...
    def forward(self, inputs: dict):
        ...
        source_credibility = self.self_attention_layer(
            torch.cat([bert_output_left.unsqueeze(1), bert_output_right.unsqueeze(1), bert_output_comment], dim=1)
        )[0]                                                                                        # torch.Size([batch_size, 2 + max_sequence_length, hidden_size])
        source_credibility = self.credibility_layer(
            source_credibility.view(source_credibility.size(0), -1))                                # torch.Size([batch_size, 64])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Self-Attention w QA Model')
    parser.add_argument('--task_name', nargs='?', default='SelfAttention_w_QA',
                        help='Task name')
