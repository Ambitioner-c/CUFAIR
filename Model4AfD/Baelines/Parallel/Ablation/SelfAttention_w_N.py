...
class OurModel(nn.Module):
    def __init__(
        ...
        self.self_attention_layer = BertSelfAttention(
            hidden_size=hidden_size,
            num_attention_heads=num_attention_heads,
        )

        self.credibility_layer = nn.Linear(hidden_size * 11, hidden_size)

        self.consensus_layer = nn.Linear(hidden_size, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        # Consensus
        source_credibility = self.self_attention_layer(
            torch.cat([bert_output_right.unsqueeze(1), bert_output_comment], dim=1)
        )[0]                                                                                        # torch.Size([batch_size, 1 + seq_length, hidden_size])
        source_credibility = self.credibility_layer(
            source_credibility.view(source_credibility.size(0), -1))                                # torch.Size([batch_size, hidden_size])

        outputs = self.consensus_layer(source_credibility)                                          # torch.Size([batch_size, num_labels])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Self-Attention w N Model')
    parser.add_argument('--task_name', nargs='?', default='SelfAttention_w_N',
                        help='Task name')
