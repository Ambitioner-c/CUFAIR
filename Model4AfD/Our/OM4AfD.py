...
from Model4AfD.LSTM.SNCILSTM import SNCILSTMModel
...
class OurModel(nn.Module):
    def __init__(
        ...
        self.sncilstm = SNCILSTMModel(
            nomination_size=hidden_size,
    ...
    def forward(self, inputs: dict):
        ...
        ping = torch.stack([x.to(self.device) for x in inputs['ping']], dim=0)                      # torch.Size([batch_size, seq_length])
        ...
        # Consensus
        consensus = self.sncilstm(bert_output_right.unsqueeze(1), bert_output_comment, ping)[:, -1, :]     # torch.Size([batch_size, hidden_size])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Our Model for AfD')
    parser.add_argument('--task_name', nargs='?', default='OM4AfD',
                        help='Task name')
