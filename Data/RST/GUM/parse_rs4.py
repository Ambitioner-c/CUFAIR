...
def main():
    filename = 'Train/GUM_academic_art.rs4'
    elements, _, _ = read_rst(filename, rel_hash={})
    # for idx in elements:
    #     print(elements[idx])
    ...
    for idx in n_s_pairs:
        print(f'N-S: {idx}, '
              f'span=({elements[idx].left}, {elements[idx].right})'
              f'-({elements[elements[idx].parent].left}, {elements[elements[idx].parent].right}), '
              f'pair={n_s_pairs[idx]}')
    ...
    for idx in s_n_pairs:
        print(f'S-N: {idx}, '
              f'span=({elements[idx].left}, {elements[idx].right})'
              f'-({elements[elements[idx].parent].left}, {elements[elements[idx].parent].right}), '
              f'pair={s_n_pairs[idx]}')

    print('=' * 100)
    for idx in n_n_pairs:
        print(f'N-N: {idx}, '
              f'span=({elements[idx].left}, {elements[idx].right}), '
              f'pair={n_n_pairs[idx]}')

    print('=' * 100)
    print(f'N-S pairs [#={len(n_s_pairs)}]:')
    print(f'S-N pairs [#={len(s_n_pairs)}]:')
    print(f'N-N pairs [#={len(n_n_pairs)}]:')
