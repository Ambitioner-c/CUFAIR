...
from transformers import DistilBertModel, set_seed, AutoTokenizer
...
class OurModel(nn.Module):
    def __init__(
        ...
        self.distilbert = DistilBertModel.from_pretrained(pretrained_model_name_or_path)
        ...
        self.relevancy_layer = nn.Linear(distilbert_hidden_size * 2, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        relevancy = self.relevancy_layer(
            torch.cat([distilbert_output_left, distilbert_output_right], dim=-1))                        # torch.Size([batch_size, 20])
...
def train(args, task_name, model, train_dataloader, dev_dataloader, epochs, lr, step):
    ...
    finetuned_distilbert_model_path = f'./FinetunedModel/{task_name}-{timestamp}/distilbert-base-uncased'
    ...
    best_model.distilbert.save_pretrained(mkdir(finetuned_distilbert_model_path))
    best_dev_result = (
    ...
    print(f'{coloring("Finetuned distilbert model path", "green_bg")}: {finetuned_distilbert_model_path}')
...
def parse_args():
    parser = argparse.ArgumentParser(description='DistilBERT Model')
    parser.add_argument('--task_name', nargs='?', default='DistilBERT',
                        help='Task name')

    parser.add_argument('--batch_size', type=int, default=4,
                        help='Batch size')
    parser.add_argument('--distilbert_hidden_size', type=int, default=768,
                        help='DistilBERT hidden size')
    ...
    parser.add_argument('--pretrained_model_path', nargs='?', default='/data/cuifulai/PretrainedModel/distilbert-base-uncased',
...
def main():
    ...
    model = OurModel(
        ...
        distilbert_hidden_size=args.distilbert_hidden_size,
