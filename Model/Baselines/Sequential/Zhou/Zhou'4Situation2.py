...
from Model.LSTM.BiGRU import BiGRUModel
...
class ZhouModel(nn.Module):
    def __init__(
        ...
        self.relevancy_layer = nn.Linear(hidden_size * 2, 64)
    ...
    def forward(self, inputs: dict):
        ...
        # AQ
        # Relevancy
        argument_quality = self.relevancy_layer(
            torch.cat([bert_output_left, bert_output_right], dim=-1))                        # torch.Size([batch_size, 64])

        # SC
        source_credibility = self.bigru(
            torch.cat([bert_output_rel_left.unsqueeze(1).expand(-1, comment.size(1), -1), bert_output_right.unsqueeze(1).expand(-1, comment.size(1), -1), bert_output_comment], dim=-1)
        )[:, -1, :]                                                                                 # torch.Size([batch_size, 64])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Zhou Model for Situation 2')
    parser.add_argument('--task_name', nargs='?', default='Zhou_BiGRU4Situation2',
                        help='Task name')
    ...
    parser.add_argument('--finetuned_model_path', nargs='?', default='./FinetunedModel/Zhou_BiGRU-20241027_213304/best_model.pth',
