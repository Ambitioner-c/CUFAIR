...
class OurModel(nn.Module):
    def __init__(
        ...
        self.mlp_ra_layer = nn.Linear(hidden_size * 2, 64)

        self.mlp_p_layer = nn.Linear(64 * 6, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        s_ra = self.mlp_ra_layer(
            torch.cat([m_r, m_a.unsqueeze(1).repeat(1, m_r.size(1), 1)], dim=-1))            # torch.Size([batch_size, max_sequence_length, 64])

        outputs = self.mlp_p_layer(
            torch.cat([argument_quality, s_ra.view(s_ra.size(0), -1)], dim=-1)
        )[:, 1].unsqueeze(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Our Model with RAHP')
    parser.add_argument('--task_name', nargs='?', default='OM_w_RAHP',
                        help='Task name')
