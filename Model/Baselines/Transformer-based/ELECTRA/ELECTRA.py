...
from transformers import ElectraModel, set_seed, AutoTokenizer
...
class OurModel(nn.Module):
    def __init__(
            self,
            freeze: bool = False,
            pretrained_model_name_or_path: str = 'electra-base-discriminator',
            device: torch.device = torch.device('cuda:0'),
            hidden_size: int = 108,
            electra_hidden_size: int = 768,
        ...
        self.relevancy_layer = nn.Linear(electra_hidden_size * 2, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        electra_output_left = self.electra(text_left)['last_hidden_state'][:, 0, :]              # torch.Size([batch_size, hidden_size])
        electra_output_right = self.electra(text_right)['last_hidden_state'][:, 0, :]            # torch.Size([batch_size, hidden_size])

        # AQ
        # Relevancy
        relevancy = self.relevancy_layer(
            torch.cat([electra_output_left, electra_output_right], dim=-1))                        # torch.Size([batch_size, 20])
...
def train(args, task_name, model, train_dataloader, dev_dataloader, epochs, lr, step):
    ...
    finetuned_electra_model_path = f'./FinetunedModel/{task_name}-{timestamp}/electra-base-discriminator'
    ...
    best_model.electra.save_pretrained(mkdir(finetuned_electra_model_path))
    best_dev_result = (
    ...
    print(f'{coloring("Finetuned electra model path", "green_bg")}: {finetuned_electra_model_path}')
...
def parse_args():
    parser = argparse.ArgumentParser(description='ELECTRA Model')
    parser.add_argument('--task_name', nargs='?', default='ELECTRA',
                        help='Task name')

    parser.add_argument('--batch_size', type=int, default=4,
                        help='Batch size')
    parser.add_argument('--electra_hidden_size', type=int, default=768,
                        help='ELECTRA hidden size')
    ...
    parser.add_argument('--pretrained_model_path', nargs='?', default='/data/cuifulai/PretrainedModel/electra-base-discriminator',
...
def main():
    ...
    model = OurModel(
        ...
        electra_hidden_size=args.electra_hidden_size,
