...
from transformers import XLNetModel, set_seed, AutoTokenizer
...
class OurModel(nn.Module):
    def __init__(
        ...
        self.relevancy_layer = nn.Linear(xlnet_hidden_size * 2, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        relevancy = self.relevancy_layer(
            torch.cat([xlnet_output_left, xlnet_output_right], dim=-1))                        # torch.Size([batch_size, 20])
...
def train(args, task_name, model, train_dataloader, dev_dataloader, epochs, lr, step):
    ...
    finetuned_xlnet_model_path = f'./FinetunedModel/{task_name}-{timestamp}/xlnet-base-cased'
    ...
    best_model.xlnet.save_pretrained(mkdir(finetuned_xlnet_model_path))
    best_dev_result = (
    ...
    print(f'{coloring("Finetuned xlnet model path", "green_bg")}: {finetuned_xlnet_model_path}')
...
def parse_args():
    parser = argparse.ArgumentParser(description='XLNet Model')
    parser.add_argument('--task_name', nargs='?', default='XLNet',
                        help='Task name')

    parser.add_argument('--batch_size', type=int, default=4,
                        help='Batch size')
    parser.add_argument('--xlnet_hidden_size', type=int, default=768,
                        help='XLNet hidden size')
    ...
    parser.add_argument('--pretrained_model_path', nargs='?', default='/data/cuifulai/PretrainedModel/xlnet-base-cased',
