...
from transformers import RobertaModel, set_seed, AutoTokenizer
...
class OurModel(nn.Module):
    def __init__(
        ...
        self.relevancy_layer = nn.Linear(roberta_hidden_size * 2, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        roberta_output_left = self.roberta(text_left)['pooler_output']              # torch.Size([batch_size, hidden_size])
        roberta_output_right = self.roberta(text_right)['pooler_output']            # torch.Size([batch_size, hidden_size])

        # AQ
        # Relevancy
        relevancy = self.relevancy_layer(
            torch.cat([roberta_output_left, roberta_output_right], dim=-1))                        # torch.Size([batch_size, 20])
...
def train(args, task_name, model, train_dataloader, dev_dataloader, epochs, lr, step):
    ...
    finetuned_roberta_model_path = f'./FinetunedModel/{task_name}-{timestamp}/roberta-base'
    ...
    best_model.roberta.save_pretrained(mkdir(finetuned_roberta_model_path))
    best_dev_result = (
    ...
    print(f'{coloring("Finetuned roberta model path", "green_bg")}: {finetuned_roberta_model_path}')
...
def parse_args():
    parser = argparse.ArgumentParser(description='RoBERTa Model')
    parser.add_argument('--task_name', nargs='?', default='RoBERTa',
                        help='Task name')

    parser.add_argument('--batch_size', type=int, default=4,
                        help='Batch size')
    parser.add_argument('--roberta_hidden_size', type=int, default=768,
                        help='RoBERTa hidden size')
    ...
    parser.add_argument('--pretrained_model_path', nargs='?', default='/data/cuifulai/PretrainedModel/roberta-base',
...
def main():
    ...
    model = OurModel(
        ...
        roberta_hidden_size=args.roberta_hidden_size,
