...
class AVERModel(nn.Module):
    def __init__(
        ...
        self.w4 = nn.Parameter(torch.randn(3, hidden_size))
        self.w5 = nn.Linear(hidden_size, hidden_size)

        self.answer_veracity_layer = nn.Linear(hidden_size * 5, num_labels)
    ...
    def forward(self, inputs: dict):
        ...
        bert_state_left = self.reduction_layer(bert_left['last_hidden_state'])                      # torch.Size([batch_size, max_length, hidden_size])
        ...
        # Attention-based Input Encoding
        v_q, v_a = self.dual_attention(bert_state_left, bert_state_right)                           # torch.Size([batch_size, hidden_size])
        v_s = self.q_2_e_attention(bert_state_left, bert_state_comment)                             # torch.Size([batch_size, max_sequence_length, hidden_size])

        # Evidence Sentence Ranking
        # QA-guided Evidence Ranking
        beta = torch.sigmoid(
            torch.einsum('bsd,bd->bs', v_s, self.w2(v_q)) +
            torch.einsum('bsd,bd->bs', v_s, self.w3(v_a)))                                     # torch.Size([batch_size, max_sequence_length])
        v_s_bar = torch.einsum('bs,bsd->bsd', beta, v_s)                                       # torch.Size([batch_size, max_sequence_length, hidden_size])

        # Evidence Agreement Matching
        gamma = torch.softmax(
            torch.max(torch.einsum('ad,bsd->bas', self.w4, torch.tanh(self.w5(v_s_bar))), dim=1)[0], dim=-1)            # torch.Size([batch_size, 3, max_sequence_length])
        v_s_tilde = torch.einsum('bs,bsd->bd', gamma, v_s_bar)                                 # torch.Size([batch_size, hidden_size])

        # Answer Veracity Prediction
        outputs = self.answer_veracity_layer(
            torch.cat([v_q, v_a, v_s_tilde, torch.abs(v_a - v_s_tilde), v_a * v_s_tilde], dim=-1))[:, 1].unsqueeze(1)                # torch.Size([batch_size, 12])
    ...
    def dual_attention(x, y):
        ...
        :param x: question [B, M, D]
        :param y: answer [B, N, D]
        :return: y-enhanced x representation, x-enhanced y representation
        """
        S = torch.matmul(x, y.transpose(-1, -2))                                        # [B, M, N]

        attention_probs_qa = nn.Softmax(dim=-1)(torch.max(S, dim=-1)[0])                # [B, M]
        attention_probs_aq = nn.Softmax(dim=-1)(torch.max(S.transpose(-1, -2), dim=-1)[0])              # [B, N]

        v_q = torch.einsum('bmd,bm->bd', x, attention_probs_qa)                    # [B, D]
        v_a = torch.einsum('bnd,bn->bd', y, attention_probs_aq)                    # [B, D]

        return v_q, v_a

    def q_2_e_attention(self, x, y):
        """

        :param x: question [B, M, D]
        :param y: evidences [B, S, N, D]
        :return:
        """
        T = torch.einsum('bsnd,bmd->bsnm', y, self.w1(x))                                     # [B, S, N, M]

        attention_probs = nn.Softmax(dim=-1)(torch.max(T, dim=-1)[0])                               # [B, S, N]

        v_s = torch.einsum('bsn,bsnd->bsd', attention_probs, y)                                # [B, S, D]
...
def parse_args():
    parser = argparse.ArgumentParser(description='AVER Model')
    parser.add_argument('--task_name', nargs='?', default='AVER',
                        help='Task name')
