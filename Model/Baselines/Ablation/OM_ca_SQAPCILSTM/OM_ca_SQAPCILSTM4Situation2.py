...
from Model.LSTM.SQAPCILSTM import SQAPCILSTMModel
...
class OurModel(nn.Module):
    def __init__(
        ...
        self.sqapcilstm = SQAPCILSTMModel(
    ...
    def forward(self, inputs: dict):
        ...
        # SC
        source_credibility = self.sqapcilstm(bert_output_rel_left.unsqueeze(1), bert_output_right.unsqueeze(1), bert_output_comment, ping)[:, -1, :]     # torch.Size([batch_size, hidden_size])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Our Model change as SQAPCILSTM for Situation 2')
    parser.add_argument('--task_name', nargs='?', default='OM_ca_SQAPCILSTM4Situation2',
                        help='Task name')
