...
from Model.LSTM.QACILSTM import QACILSTMModel
...
class OurModel(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        # SC
        source_credibility = self.qacilstm(bert_output_rel_left.unsqueeze(1), bert_output_right.unsqueeze(1), bert_output_comment, ping)[:, -1, :]     # torch.Size([batch_size, hidden_size])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Our Model change as QACILSTM for Situation 2')
    parser.add_argument('--task_name', nargs='?', default='OM_ca_QACILSTM4Situation2',
                        help='Task name')
