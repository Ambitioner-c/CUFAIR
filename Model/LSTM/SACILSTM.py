...
class SACILSTMCell(nn.Module):
    ...
    def forward(
        ...
        gates = a_temp + torch.mm(x, self.weight_ih) + self.bias_ih + torch.mm(h_t_minus_1, self.weight_hh) + self.bias_hh
...
class SACILSTM(nn.Module):
    def __init__(
        ...
        layers = [SACILSTMCell(answer_size, input_size, hidden_size, is_peephole)]
        for _ in range(self.num_layers - 1):
            layers += [SACILSTMCell(answer_size, hidden_size, hidden_size, is_peephole)]
    ...
    def forward(
        ...
        for i, cell in enumerate(self.net):  # Layers
            ...
            for t in range(x.size(0)):  # Sequences
                ...
                h_t, c_t = cell(answer[0], inputs[t], (h_t, c_t))
...
class SACILSTMModel(nn.Module):
    def __init__(
        ...
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.sacilstm = SACILSTM(answer_size, input_size, hidden_size, num_layers, num_attention_heads, batch_first=True, is_peephole=is_peephole, ci_mode=ci_mode)
        self.fc = nn.Linear(hidden_size, output_size)

    def forward(self, answer: Tensor, x: Tensor, ping: Tensor) -> Tensor:
        out, _ = self.sacilstm(answer, x, ping, None)
        out = self.fc(out)
...
def main():
    ...
    outputs = model(answers, inputs, pings)
    print(outputs.size())
